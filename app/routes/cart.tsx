          ...
          <div className="space-y-4">
            {cartItems.map((item) => (
              <div
                ...
                className="flex relative items-center space-x-4 py-4 border-b"
              >
                <div className="flex items-center gap-2 lg:gap-4">
                  <Checkbox
                    ...
                    onCheckedChange={() =>
                      handleToggleSelect(item._id.toString())
                    }
                  />
                ...
                <div className="flex w-full ga-2.5 items-center justify-between max-lg:flex-col max-lg:items-start">
                  <div className="flex-grow">
                    <h3 className="font-semibold line-clamp-1">{item.title}</h3>
                    <p className="text-zinc-600 line-clamp-1 lg:line-clamp-2">
                      {item.description}
                    </p>
                  </div>
                ...
                <Button
                  variant="ghost"
                  className="size-auto hover:text-zinc-700 text-xs hover:bg-white absolute top-0 right-0 text-zinc-500"
                  size="icon"
        ...
        <div className="bg-zinc-50 max-lg:hidden p-6 rounded-xl sticky top-10">
          ...
          <div className="mt-8">
            <a aria-disabled={total === 0} href="/checkout">
              <Button className="w-full mb-2.5">Confirm payment</Button>
            </a>
            <a href="/">
...
export const loader: LoaderFunction = async ({ request }) => {
  try {
    ...
    const userCart = await db
      .collection("carts")
      .findOne({ _id: new ObjectId(user.cartId as string) });
      ...
      .find({
        _id: {
          $in: userCart.products.map(
            (product: any) => new ObjectId(product.productId as string)
          ),
        },
      })
      .toArray();
